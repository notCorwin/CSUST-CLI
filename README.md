...
| `classrooms`, `selections`, `course-selection`, `special-course-query`, `social-exam-registration`, `make-up-exam-registration`, `terms`, `semester-start` | 教室、选课和考试报名信息；跨专业选修使用 `--scope cross-major`，特殊选课查询使用 `--term` 和可选 `--special-name` |
