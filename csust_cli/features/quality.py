...
def register(subparsers: argparse._SubParsersAction) -> None:
    ...
    public_action = public_children.add_parser("action", help="按页面动作 ref 或序号执行找回密码页面动作")
    ...
    action = children.add_parser("action", aliases=["run"], help="按页面动作 ref 或序号执行链接、脚本或表单动作")
    action.add_argument("--path", required=True, help="教学质量保障系统页面路径")
    action.add_argument("--index", type=int, help="兼容模式：quality get 输出的动作序号")
    action.add_argument("--ref", help="quality get 输出的稳定动作 ref")
    ...
    action.add_argument("--fingerprint", help="使用 --index 时校验 quality get 返回的页面指纹")
