...
func (a NativeSite) serviceHallDictionary(ctx context.Context, operation string, args []string) (map[string]any, *siteError) {
	...
	if operation == "departments" {
		...
		categoryID, found, categoryErr := businessValue(args, "--category-id")
		if categoryErr != nil {
			return nil, categoryErr
		}
		if !found || strings.TrimSpace(categoryID) == "" {
			categoryID = "0"
		}
	...
	return map[string]any{
		...
		"query": map[string]any{"category_id": body["classifyId"], "favorites": isCollect == 1},
	}, nil
}
