...
var businessServices = []businessService{
	...
	{"transport-mobile", "交通运输工程综合信息", "transport-mobile", "学院管理", "high", "live WiJat SPA defines token authentication, user profile, pending count, password change, protected defense and finance project/item query, detail and export plus finance-item create/update/delete, achievement create/update/status, KPI and notice create/update, note, access, workflow, vacation and workroom attendance/tree APIs"},
	...
	{"campus-card", "校园卡", "campus-card", "校园服务", "medium", "服务大厅当前目录将校园卡映射到 yktfw.csust.edu.cn:8090/ias/prelogin；当前网络探测未取得可验证的卡务 API"},
...
func (a NativeSite) executeBusinessCommand(ctx context.Context, args []string) (map[string]any, *siteError) {
	...
	case "training-platform", "peixun":
		return a.executeTrainingPlatform(ctx, args[1:])
	case "ehall":
	...
	case "mooc":
		return a.executeMooc(ctx, args[1:])
	case "quality-system":
		return a.executeQualitySystem(ctx, args[1:])
	case "party-exam":
	...
	case "campus-card":
		return a.executeServiceStatusCommand(ctx, args[1:], "campus-card", "校园卡")
	...
	case "campus-map":
		return a.executeCampusMap(ctx, args[1:])
	case "continuing-education":
		return a.executeContinuingEducation(ctx, args[1:])
	case "virtual-lab":
		return a.executeVirtualLab(ctx, args[1:])
	case "library-center":
		return a.executeSSOServiceCommand(ctx, args[1:], libraryPersonalService, "图书馆个人中心")
...
func businessAllowedFlags(service, operation string) map[string]bool {
	...
	case "transport-mobile":
		...
		case "kpi-update":
			add("--id", "--owner-id", "--year", "--type", "--block", "--name", "--score", "--remark", "--info", "--yes")
		case "notice-create":
			add("--title", "--type", "--content", "--public", "--private", "--recipient-id", "--audience-user-id", "--audience-group-id", "--audience-role-id", "--approver-id", "--url", "--file-id", "--need-approval", "--no-approval", "--yes")
		case "notice-update":
			add("--id", "--title", "--type", "--content", "--public", "--private", "--recipient-id", "--audience-user-id", "--audience-group-id", "--audience-role-id", "--approver-id", "--url", "--file-id", "--need-approval", "--no-approval", "--yes")
		case "note":
			add("--id")
		case "note-create":
		...
		case "defense", "finance", "finance-item":
			add("--id")
		case "defense-export", "finance-export":
			add("--id", "--output")
		...
		case "finance-item-create":
			add("--project-id", "--money", "--direction", "--type", "--remark", "--file", "--yes")
		case "finance-item-update":
			add("--id", "--money", "--direction", "--type", "--remark", "--file", "--file-id", "--clear-files", "--yes")
	...
	case "campus-network":
		...
		case "bills", "billing":
			add("--year")
		case "payments", "usage", "online-usage", "operations", "operator-log":
			add("--date", "--from", "--to", "--month")
		case "change-password":
			add("--new-password", "--new-password-stdin", "--password-confirm", "--yes")
		case "update-profile":
			add("--phone", "--email", "--install-address", "--anti-fraud", "--yes")
		case "package", "reserve-package":
			add("--user-type", "--network", "--bandwidth", "--login-method", "--billing-type", "--terminals", "--cost", "--yes")
		case "set-devices", "bind-devices":
			add("--mac", "--yes")
		case "unbind-device", "disconnect", "offline":
			add("--index", "--yes")
		}
	case "campus-card":
		common()
	case "student-digital-archive":
		common()
		add("--access-token", "--user-id", "--student-id")
		switch operation {
		case "login":
			add("--auth", "--username", "--password-stdin", "--captcha", "--captcha-image")
		case "logout":
		...
		case "forms", "records":
			add("--category")
		case "schedule":
			add("--term", "--page", "--page-size")
		case "borrow":
			add("--status", "--page", "--page-size")
		case "internet-usage", "internet-flow", "consumption-details":
			add("--year", "--month", "--page", "--page-size")
		case "notes":
	...
	case "campus-map":
		...
		case "types", "categories":
			add("--campus", "--category")
		case "points":
			add("--campus", "--type", "--type-code")
		case "point", "detail":
			add("--id")
		case "search":
			add("--campus", "--keyword", "--page", "--page-size")
		case "panoramas", "roam":
			add("--campus", "--kind")
		case "stats":
			add("--campus")
	...
	case "service-hall":
		...
		case "guide", "service", "detail":
			add("--id", "--category-id", "--department-id")
		case "favorite", "unfavorite":
			add("--service-id", "--yes")
		case "rating", "service-rating":
			add("--service-id", "--page", "--page-size")
		case "my-rating":
			add("--service-id")
		case "rate":
			add("--service-id", "--rating", "--comment", "--yes")
